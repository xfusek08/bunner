...
type ExtractOptionType<D extends OptionDefinition> = D extends SpecificOptionDefinition<infer T> ? T : never;

type IsBooleanOption<D extends OptionDefinition> = D extends { type: 'boolean' } ? true : false;

type HasDefaultValue<D extends OptionDefinition> = D extends { defaultValue: OptionValue<ExtractOptionType<D>> } ? true : false;
...
type EvaluatedOptions<T extends OptionDefinitions> =
    T extends readonly [infer First, ...infer Rest]
        ? First extends OptionDefinition
            ? EvaluatedOption<First> & EvaluatedOptions<
                Rest extends OptionDefinitions
                    ? Rest
                    : []
                >
            : {}
...
export function hasDefaultValue<D extends OptionDefinition>(option: D): option is WithDefaultValue<D> {
