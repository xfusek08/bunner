...
export default class Option<T extends OptionType = OptionType> {
    ...
    public static create<T extends OptionType = OptionType>(
        ...
        return new Option(
            ...
            definition.type,
            'required' in definition ? definition.required : false,
    ...
    public get identifier(): string {
        ...
        throw new Error(
            'Unknown Error: Option does not have neither long nor short name - This should never happen',
        );
    }
    ...
    public parseValue(value: string): Error | OptionValue<T> {
        ...
        if (this.isType('number')) {
            ...
            if (Number.isNaN(parsed)) {
                return new Error(
                    `Option ${this.dashedIdentifier} must be a valid number representation. Received: "${value}"`,
                );
            }
